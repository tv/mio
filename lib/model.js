...
AbstractModel.use = function(method, plugin) {
  ...
  if (method) {
    this.use[method] = (this.use[method] || [])
    this.use[method].push(plugin);
  }
  else {
...
AbstractModel.find = function(query, callback) {
  ...
  (function next(err, model) {
    ...
    if (model || i === Model.use.find.length) {
      done(null, model);
    }
    else {
      Model.use.find[i++].call(Model, query, next);
    }
  })();
...
AbstractModel.findAll = function(query, callback) {
  ...
  (function next(err, collection) {
    ...
    if ((collection && collection.length) || (i === Model.use.findAll.length)) {
      done(null, collection);
    }
    else {
      Model.use.findAll[i++].call(Model, query, next);
    }
  })();
...
AbstractModel.count = function(query, callback) {
  ...
  (function next(err, count) {
    ...
    if (count !== undefined || i === Model.use.count.length) {
      done(null, count);
    }
    else {
      Model.use.count[i++].call(Model, query, next);
    }
  })();
...
model.save = function(callback) {
  ...
  (function next(err) {
    ...
    if (i === Model.use.save.length) {
      done(null);
    }
    else {
      Model.use.save[i++].call(model, changed, next);
    }
  })();
...
model.remove = function(callback) {
  ...
  (function next(err) {
    ...
    if (i === Model.use.remove.length) {
      done();
    }
    else {
      Model.use.remove[i++].call(model, next);
    }
  })();
