...
exports.createModel = function(type, options) {
  ...
  if (options.thunks || options.promises) {
    ...
    Model.removeAll = somethingify(Model.removeAll);
...
AbstractModel.removeAll = function(query, callback) {
  ...
  // Call each store's removeAll method.
  (function next(err) {
    if (err) return done(err);

    if (i === Model.stores.length) {
      done();
    }
    else {
      Model.stores[i++].removeAll.call(Model, query, next);
    }
  })();

  // Emit `after removeAll` event.
  function done(err) {
    if (err) return callback.call(this, err);

    Model.emit('after removeAll');
